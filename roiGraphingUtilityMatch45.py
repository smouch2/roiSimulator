...
def simulate_game(start_date, end_date, numbers, reselect=False):
    ...
    return total_spent, total_won, roi, match4plus
...
def main():
    ...
    match_user_cumulative = np.zeros(num_simulations)  # Store matches for user-selected scenario
    match_random_cumulative = np.zeros(num_simulations)  # Store matches for randomly-selected scenario
    match_reselected_cumulative = np.zeros(num_simulations)  # Store matches for reselected every day scenario

    for i in range(num_simulations):
        
        # User-selected scenario
        _, _, _, match_user = simulate_game(start_date, end_date, numbers)
        match_user_cumulative[i] = match_user_cumulative[i-1] + match_user if i > 0 else match_user

        # Randomly-selected scenario
        numbers = random.sample(range(1, 40), 5)
        _, _, _, match_random = simulate_game(start_date, end_date, numbers)
        match_random_cumulative[i] = match_random_cumulative[i-1] + match_random if i > 0 else match_random

        # Reselected every day scenario
        _, _, _, match_reselected = simulate_game(start_date, end_date, numbers, reselect=True)
        match_reselected_cumulative[i] = match_reselected if i == 0 else match_reselected_cumulative[i-1] + match_reselected
        

    # Plot the results
    plt.plot(match_user_cumulative, label='User-selected numbers')
    plt.plot(match_random_cumulative, label='Randomly-selected numbers')
    plt.plot(match_reselected_cumulative, label='Reselected numbers every day')
    plt.legend(loc='upper right')
    plt.xlabel('Simulation Run')
    plt.ylabel('Number of 4 or 5 matches')
    plt.title('Number of 4 or 5 Matches for Different Scenarios over Multiple Simulations')
